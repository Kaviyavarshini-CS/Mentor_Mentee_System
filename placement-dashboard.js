    ...
    async function loadDashboardData() {
        try {
            ...
            const [dashboardRes, updatesRes, statusRes] = await Promise.all([
                fetch('http://127.0.0.1:80/api/dashboard/placement-officer', {
                    headers: {
    ...
    async function viewStudentDetails(studentId) {
        try {
            const [studentRes, placementRes] = await Promise.all([
                fetch(`http://127.0.0.1:80/api/students/${studentId}`, {
                    headers: {
                        'Authorization': `Bearer ${token}`
                    }
                }),
                fetch(`http://127.0.0.1:80/api/placement-status?student_id=${studentId}`, {
                    headers: {
                        'Authorization': `Bearer ${token}`
                    }
                })
