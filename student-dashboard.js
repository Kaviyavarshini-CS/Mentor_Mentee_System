document.addEventListener('DOMContentLoaded', function() {
    ...
    async function loadTasks(filter = 'all') {
        try {
            ...
            const filteredTasks = filter === 'all' ? tasks : tasks.filter(t => t.student_status === filter);
            
            filteredTasks.forEach(task => {
                const taskCard = document.createElement('div');
                taskCard.className = `col-md-6 col-lg-4 mb-4 task-card task-status-${task.student_status}`;
                ...
                taskCard.innerHTML = `
                    <div class="card h-100">
                        <div class="card-body">
                            ...
                            <div class="mb-2">
                                <span class="badge ${getStatusBadgeClass(task.student_status)}">${formatStatus(task.student_status)}</span>
                                ${task.deadline && new Date(task.deadline) < new Date() && task.student_status !== 'completed' ? 
                                    '<span class="badge bg-danger ms-1">Overdue</span>' : ''}
                            </div>
                            <p class="card-text"><small class="text-muted">Mentor: ${task.mentor_name}</small></p>
                            <p class="card-text"><small class="text-muted">Deadline: ${new Date(task.deadline).toLocaleString()}</small></p>
